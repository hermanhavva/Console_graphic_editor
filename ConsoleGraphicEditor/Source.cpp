...
int main() 
  {
	...
	unique_ptr<Program> program = make_unique<Program>(hout, polygonPos, 80, 20, menuPos, yellowFontBlackText);
	unique_ptr<Figure> startFigure = make_unique<Rectangle2>(startPos, 0,0, yellowFontBlackText);
	while (true) 
	{
		try  
		{
